...
local LockableEquipment = {
    ...
    ["Head"] = T{"Reraise Hairpin", "Dream Hat +1", "Chef's Hat"},
    ["Neck"] = T{"Opo-opo Necklace"},
    ["Ear1"] = T{"Reraise Earring"},
    ["Ear2"] = T{"Reraise Earring"},
    ["Body"] = T{"Custom Gilet +1", "Custom Top +1", "Magna Gilet +1", "Magna Top +1", "Savage Top +1", "Elder Gilet +1", "Wonder Maillot +1", "Wonder Top +1", "Mandra. Suit", "Culinarian's Apron", "Fisherman's Apron"},
    ["Hands"] = T{"Angler's Gloves"},
    ...
    ["Back"] = T{},
    ["Waist"] = T{},
    ["Legs"] = T{"Angler's Hose"},
    ["Feet"] = T{"Powder Boots", "Waders"}
}
...
function gcinclude.Load()
    ...
    local function delayLoad()
        ...
        AshitaCore:GetChatManager():QueueCommand(-1, "/addon load ticker")
    end
    delayLoad:once(3)
end
...
function gcinclude.DoDefaultOverride(isMelee)
    ...
    if (player.Status == "Resting") then
        ...
        if (os.clock() > restTimestamp + 19) then
