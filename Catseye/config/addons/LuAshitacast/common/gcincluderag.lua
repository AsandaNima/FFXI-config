...
local LockableEquipment = {
    ...
    ["Ammo"] = T{"Sinking Minnow"},
    ["Head"] = T{"Reraise Hairpin", "Dream Hat +1", "Chef's Hat", "Sprout Beret", "Midras's Helm +1", "Miner's Helmet"},
    ["Neck"] = T{"Opo-opo Necklace","Field Torque"},
    ["Ear1"] = T{"Reraise Earring"},
    ["Ear2"] = T{"Reraise Earring"},
    ["Body"] = T{"Custom Gilet +1", "Custom Top +1", "Magna Gilet +1", "Magna Top +1", "Savage Top +1", "Elder Gilet +1", "Wonder Maillot +1", "Wonder Top +1", "Mandra. Suit", "Culinarian's Apron", "Fisherman's Tunica", "Plain Tunica"},
    ["Hands"] = T{"Fsh. Gloves","Plain Gloaves"},
    ["Ring1"] = T{"Anniversary Ring", "Emperor Band", "Chariot Band", "Empress Band", "Homing Ring", "Tavnazian Ring", "Dem Ring", "Holla Ring", "Mea Ring", "Altep Ring", "Yhoat Ring", "Warp Ring", "Echad Ring", "Provenance Ring"},
    ["Ring2"] = T{"Anniversary Ring", "Emperor Band", "Chariot Band", "Empress Band", "Homing Ring", "Tavnazian Ring", "Dem Ring", "Holla Ring", "Mea Ring", "Altep Ring", "Yhoat Ring", "Warp Ring", "Echad Ring", "Provenance Ring"},
    ["Back"] = T{},
    ["Waist"] = T{"Field Rope"},
    ["Legs"] = T{"Fisherman's Hose", "Plain Hose"},
    ["Feet"] = T{"Powder Boots", "Fisherman's Boots","Plain Boots"}
}
...
local Commands = T{ "kite","lock","locktp","lockset","warpme" }
...
function gcinclude.DoDefaultOverride(isMelee)
    ...
    if (player.IsMoving == true) and (player.Status ~= "Engaged") then
        if (kupo_suit == true) then
            gFunc.Equip("Body", "Kupo suit")
        end
    end

    if (player.MainJobLevel <= 50) and (player.MPP < 95) then
        gFunc.Equip("Body", "Garirson Runica +1")
    end

    if (player.MainJobLevel <= 50) and (player.HPP < 95) then
...
function gcinclude.BuildLockableSet(equipment)
    ...
    for slot, item in pairs(equipment) do
        if (LockableEquipment[slot]:contains(item.Name)) then
            ...
            elseif (item.Name == "Mandra. Suit" or item.Name == "Kupo suit") then
