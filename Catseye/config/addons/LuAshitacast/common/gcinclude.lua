...
gcinclude.BstPetMagicAccuracy = T({
	"Toxic Spit",
	"Acid Spray",
	"Leaf Dagger",
	"Venom Spray",
...
gcinclude.HelixSpells =
	T({ "Ionohelix", "Cryohelix", "Pyrohelix", "Geohelix", "Anemohelix", "Hydrohelix", "Luminohelix", "Noctohelix" })
gcinclude.StormSpells =
	T({ "Thunderstorm", "Hailstorm", "Firestorm", "Sandstorm", "Windstorm", "Rainstorm", "Aurorastorm", "Voidstorm" })
gcinclude.NinNukes = T({
	"Katon: Ichi",
	"Katon: Ni",
	"Katon: San",
	"Hyoton: Ichi",
...
function gcinclude.HandleCommands(args)
	if not gcinclude.AliasList:contains(args[1]) then
		return
	end
	...
	elseif args[1] == "craft" then
		gcinclude.craft = false
		if args[2] ~= nil then
			gcdisplay.SetCycle(args[1], args[2])
			toggle = "Crafting Set"
			status = gcdisplay.GetCycle("craft")
			print(chat.header("GCinclude"):append(chat.message("Crafting directly set to " .. status)))
		else
			gcdisplay.AdvanceCycle("craft")
			toggle = "Crafting Set"
			status = gcdisplay.GetCycle("craft")
			print(chat.header("GCinclude"):append(chat.message("Crafting set to " .. status)))
		end
		if status ~= "None" then
			gcinclude.craft = true
		end
	...
	elseif args[1] == "mpset" then
		gcinclude.MPSet = not gcinclude.MPSet
		toggle = "MP Set"
		status = gcinclude.MPSet
	end
	if
		(player.MainJob == "RDM")
		or (player.MainJob == "BLM")
		or (player.MainJob == "SCH")
		or (player.MainJob == "GEO")
	then
...
function gcinclude.SetRegenRefreshGear()
	if gcinclude.settings.AutoGear == false then
		return
	end
...
function gcinclude.CheckWsBailout()
	...
	if
		gcinclude.settings.WScheck
		and not gcinclude.DistanceWS:contains(ws.Name)
		and (tonumber(target.Distance) > gcinclude.settings.WSdistance)
	then
		print(
			chat.header("GCinclude")
...
function gcinclude.DoExpRing()
	AshitaCore:GetChatManager():QueueCommand(1, '/lac equip ring2 "Empress Band"')

	local function usering()
		local function forceidleset()
			AshitaCore:GetChatManager():QueueCommand(1, "/lac set Idle")
		end
		AshitaCore:GetChatManager():QueueCommand(1, '/item "Empress Band" <me>')
		forceidleset:once(5)
	end
...
function gcinclude.DoCORmsg(roll)
	if gcinclude.CORmsg == false then
		return
	end
...
function gcinclude.DoAspir()
	...
	if
		(player:GetMainJob() == 4 and player:GetJobPointsSpent(4) >= 550)
		or (player:GetMainJob() == 21 and player:GetJobPointsSpent(21) >= 550)
	then
		if recast3 == 0 then
	...
	elseif
		(player:GetMainJob() == 4 and player:GetMainJobLevel() >= 83)
		or (player:GetMainJob() == 8 and player:GetMainJobLevel() >= 78)
		or (player:GetMainJob() == 20 and player:GetMainJobLevel() >= 97)
		or (player:GetMainJob() == 21 and player:GetMainJobLevel() >= 90)
	then
		if recast2 == 0 then
			AshitaCore:GetChatManager():QueueCommand(1, '/ma "Aspir II" <t>')
		elseif recast1 == 0 then
...
function gcinclude.CheckDefault()
	...
	if gcinclude.ZeniSet == true then
		gFunc.EquipSet(gcinclude.sets.Zeni)
	end
	if gcinclude.MPSet == true then
		gFunc.EquipSet(gcinclude.sets.MPset)
	end
	if gcinclude.RRSET == true then
		gFunc.EquipSet(gcinclude.sets.Reraise)
	end
	if gcinclude.craft == true then
		status = gcdisplay.GetCycle("craft")
		if player.Status ~= "Engaged" then
			gFunc.EquipSet(gcinclude.sets[status])
		end
		-- 		print(chat.header('GCinclude'):append(chat.message("Equiping " .. status .. " set.")));
	end
