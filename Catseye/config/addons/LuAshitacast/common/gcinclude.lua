...
gcinclude.LockingRings = T{"Provenance Ring", 'Empress Band', 'Trizek Ring', 'Endorsement Ring', 'Capacity Ring', 'Warp Ring','Facility Ring','Dim. Ring (Dem)','Dim. Ring (Mea)','Dim. Ring (Holla)', "Prouesse Ring", "Craftmaster's Ring"};
...
function gcinclude.SetVariables()
	...
	-- set defense mode
	gcdisplay.CreateCycle("craft", {[1] = "none",
									[2] = "alchemy",
									[3] = "bonecraft",
									[4] = "clothcraft",
									[5] = "cooking",
...
function gcinclude.HandleCommands(args)
	...
	elseif (args[1] == "craft") then
		...
		if status ~= "None" then gcinclude.craft = true end;
...
function gcinclude.SetRegenRefreshGear()
	...
	if (player.Status == 'Idle') then
		if (player.HPP < gcinclude.settings.RegenGearHPP ) then
			if (gcinclude.settings.CurrentLevel <= 50) then gFunc.Equip("Head", "Garrison Sallet +1") end
...
function gcinclude.CheckDefault()
	...
	if (gcinclude.MPSet == true) then gFunc.EquipSet(gcinclude.sets.MPset) end
